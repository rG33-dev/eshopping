...
fun EachCategoryProductScreenUi(
    productID: String, // This is actually the category name based on the NavHost usage
    navController: NavController,
    viewModel: ShoppingAppViewModel = hiltViewModel()
) {
    val state by viewModel.getSpecificCategoryItemsState.collectAsState()
                        ...
                        items(products) { product ->
                            DetailedProductCard(
                                product = DetailedProductItem(
                                    image = R.drawable.ic_launcher_foreground, // Note: Model has String URL, Card expects Int. Using placeholder.
                                    productName = product.name,
                                    productPrice = "₹${product.price}",
                                    discountedPrice = "₹${product.finalPrice}",
                                    discount = "${calculateDiscount(product.price, product.finalPrice)}% off",
                                    rating = "4.2", // Mocked as not in data model
                                    ratingCount = "(123)" // Mocked as not in data model
                                ),
                                modifier = Modifier
                                    .padding(4.dp)
                                    .clickable {
